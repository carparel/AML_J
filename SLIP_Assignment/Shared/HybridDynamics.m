...
%
function [p,yOUT, zOUT, tOUT, varargout] = HybridDynamics(yIN, zIN, p, varargin)
...
%     end
    ...
    function dydt_ = ODE(~,y_)
        ...
        % (which must be on the MATLAB search path):
        if isempty(exctFcnHndl)
            [dydt_,p] = FlowMap(y_, zIN, p);
        else
            [dydt_,p] = FlowMap(y_, zIN, p, exctFcnHndl, s);
        end
    end
    % End ODE
    % *********************************************************************
